...
class SaasPaymentPageSection extends Component
{
    ...
    public function render()
    {
        ...
        $this->cart->save();
        $methods = PaymentMethod::where('active_status', 1)->where('module_status', 1)->where('method', '!=', 'Bank Payment')->where('method', '!=', 'Offline Payment')->where('method', '!=', 'Wallet')->get(['method', 'logo']);
